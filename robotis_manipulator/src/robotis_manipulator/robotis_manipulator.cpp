...
bool RobotisManipulator::solveVisualInverseKinematics(Name tool_name, std::vector<Eigen::Vector3d>* target, std::vector<JointValue> *goal_joint_value, Eigen::Matrix3d goal_orientation)
{
  if(kinematics_added_state_){
    ...
    log::warn("[solveVisualInverseKinematics] Kinematics Class was not added.");
    return false;
  }
}
                                            ...
                                            std::vector<JointValue> present_joint_value)
{
  ...
  bool success = kinematics_->solveVisualInverseKinematics(trajectory_.getManipulator(), trajectory_.getPresentControlToolName(), &target, &goal_joint_value, goal_orientation);
  ...
  {
    log::error("[TASK_TRAJECTORY] Fail to solve visual IK");
    return false;
  }
}
