  ...
  with_flashlight_(false)
{
  ...
  ************************************************************/
  ...
  with_flashlight_      = priv_node_handle_.param<bool>("with_flashlight", false);
...
void OpenManipulatorController::initServer()
{
  ...
  visual_goal_task_space_path_server_                  = node_handle_.advertiseService("visual_goal_task_space_path", &OpenManipulatorController::visualGoalTaskSpacePathCallback, this);
  visual_goal_task_space_path_with_orientation_server_    = node_handle_.advertiseService("visual_goal_task_space_path_with_orientation", &OpenManipulatorController::visualGoalTaskSpacePathWithOrientationCallback, this);
...
bool OpenManipulatorController::visualGoalTaskSpacePathCallback(open_manipulator_msgs::SetKinematicsPose::Request  &req,
                                                                open_manipulator_msgs::SetKinematicsPose::Response &res)
{
  ...
  if (!open_manipulator_.makeTaskTrajectory(req.end_effector_name, target, req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
}

bool OpenManipulatorController::visualGoalTaskSpacePathWithOrientationCallback(open_manipulator_msgs::SetKinematicsPose::Request  &req,
                                                                                open_manipulator_msgs::SetKinematicsPose::Response &res)
{
  Eigen::Quaterniond q(req.kinematics_pose.pose.orientation.w,
                       req.kinematics_pose.pose.orientation.x,
  ...
  if (!open_manipulator_.makeTaskTrajectory(req.end_effector_name, target, orientation, req.path_time))
    res.is_planned = false;
  else
    res.is_planned = true;
}
...
{
  open_manipulator_.processOpenManipulator(time, using_platform_, with_gripper_, with_flashlight_);
}
